const calculateMinEuclidDistance = (curNode, hidersAnnouncedPos) => {
  ...
  hidersAnnouncedPos.forEach((position) => {
    let distance =
      Math.pow(position.row - curNode.row, 2) +
      Math.pow(position.col - curNode.col, 2);
...
export const calculateMinEuclidDistanceForHider = (curNode, finderLocation) => {
  let distance =
    Math.pow(finderLocation.row - curNode.row, 2) +
    Math.pow(finderLocation.col - curNode.col, 2);
